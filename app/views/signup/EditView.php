    ...
    <body>
            ...
            <div id="signup-modal" class="modal fade">
                <div class="modal-header">
                    ...
                    <h3 id="modal-title">Edit user information</h3>
                </div>
                <div class="modal-body" id="modal-contents">
                    <p class="lead">Edit item you want to change.</p>
                    <form class="form-horizontal">
                        <div class="control-group">
                            <label class="control-label" for="inputEmail">USER NAME</label>
                            <div class="controls">
                            <input type="text" id="user" placeholder="<?php print($this->get('user')); ?>">
                            </div>
                        </div>
                        <div class="control-group">
                            <label class="control-label" for="inputPassword">EMAIL </label>
                            <div class="controls">
                            <input type="text" id="email" placeholder="<?php print($this->get('email')); ?>">
                            </div>
                        </div>
                    </form>
                    <blockquote>
                        <p>If you change username, your alert-list and password is gone.</br></p>
                        <p>ユーザ名を変更すると、現在のパスワード及びアラートリストは失われます。<br/></p>
                        <p>その場合は変更後に再度設定してください。<br/></p>
                        <p>変更が反映されるまで多少時間がかかります。お急ぎの場合は管理者まで。<br/></p>
                    </blockquote>
                    <p id="modal-error" class="text-error"></p>
                </div>
                <div class="modal-footer">
                    <a href="<?php print($this->get('url')); ?>"  class="btn">BACK</a>
                    <a id="signup" class="btn btn-primary">SAVE</a>
                </div>
            </div>
        ...
        <script>
                ...
                var Auth = function(un, ps, tk) {
                    var defer = $.Deferred();
                    $.ajax({
                        url: "<?php print($this->get('url')); ?>/signup/editajax",
                            type: "POST",
                            data: {
                                user: un,
                                email: ps,
